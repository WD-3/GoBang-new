...
def api_delete_game(game_id: str):
    ...
    if game_id in llm_players:
        ...
        print(f"[DELETE] Cleaned up LLM players for game {game_id}")
    if game_id in games:
        del games[game_id]
    if game_id in llm_players:
        del llm_players[game_id]
...
def api_auto_move(game_id: str):
    ...
    TIMEOUT_SECONDS = 120  # 超时时间 120 秒（2 分钟）
    ...
    for i, move in enumerate(moves):
        ...
        if i > 0:  # 第一步不分析
            ...
            # 检查是否未阻挡必杀：对手有活四或冲四但没有堵
                ...
                if (
                    new_opp_patterns.get(PatternType.OPEN_FOUR, 0) > 0
                    or new_opp_patterns.get(PatternType.RUSH_FOUR, 0) > 0
                ):
                    # 对手仍然有活四/冲四，没有堵住
                    is_error = True
            ...
            # 检查是否错过发展活三：自己有活三但没有发展成活四/冲四
                ...
                if (
                    new_patterns.get(PatternType.OPEN_FOUR, 0) == 0
                    and new_patterns.get(PatternType.RUSH_FOUR, 0) == 0
                    and not has_opp_open_three
                ):  # 如果对手也有活三，可能需要先防守
                    is_error = True
            ...
            # 检查是否未阻挡对手活三
                ...
                test_engine = PatternEngine(test_board, game.rule_mode)
                new_opp_patterns = test_engine.count_patterns(opponent_stone)
                if new_opp_patterns.get(PatternType.OPEN_THREE, 0) >= opp_patterns.get(
                    PatternType.OPEN_THREE, 0
                ):
                    # 对手的活三没有被堵住
                    # 但如果自己这步形成了更大威胁，不算失误
                    my_new_patterns = test_engine.count_patterns(current_stone)
                    if (
                        my_new_patterns.get(PatternType.OPEN_FOUR, 0) == 0
                        and my_new_patterns.get(PatternType.RUSH_FOUR, 0) == 0
                    ):
                        is_error = True
...
def _setup_llm_players(game_id: str, game: Game):
    ...
    print(f"[_setup_llm_players] Creating NEW LLM players for game {game_id} at {time.time()}")
    ...
    if game.black_player.player_type == PlayerType.LLM:
        ...
        print(f"[LLM] BLACK player created: model={model}, temp={temperature}, game={game_id}")
    ...
    if game.white_player.player_type == PlayerType.LLM:
        ...
        model = config.get("model", "gemma3:1b")
        temperature = config.get("temperature", 0.1)
        print(f"[LLM] WHITE player created: model={model}, temp={temperature}, game={game_id}")
