...
class LLMPlayer:
    ...
    MOVE_PROMPT_TEMPLATE = """你是五子棋(Gomoku/Five-in-a-Row)玩家。You are a Gomoku player.
棋盘大小/Board size: {size}×{size}

当前棋盘/Current Board (·=空/empty, ●=黑/black, ○=白/white):
{board_visual}

你是{color}方。You are playing as {color_en}.
{threat_analysis}
{candidates_info}

【规则/Rules】
- 先连成5子者胜(横/竖/斜) / First to connect 5 stones wins (horizontal/vertical/diagonal)
- 黑先 / Black moves first

【策略优先级/Strategy Priority】
1. 己方有四连 → 立即落子获胜 / You have 4 in a row → WIN NOW
2. 对手有四连 → 必须防守 / Opponent has 4 in a row → MUST BLOCK
3. 己方有活三 → 进攻 / You have open-3 → ATTACK
4. 对手有活三 → 防守 / Opponent has open-3 → DEFEND

只输出一个坐标(如/H8)。Output only ONE coordinate (e.g., "H8"): """
    ...
    def _analyze_threats(self, game: Game) -> str:
        ...
        if my_winning:
            lines.append(
                f"🎯 必胜/IMMEDIATE WIN: {', '.join(p.to_notation() for p in my_winning[:3])}"
            )

        opp_winning = engine.get_winning_moves(opp_stone)
        if opp_winning:
            lines.append(
                f"🚨 必防/MUST BLOCK: {', '.join(p.to_notation() for p in opp_winning[:3])}"
            )

        vcf = VCFEngine(board, my_stone)
        vcf_result = vcf.search()
        if vcf_result and not my_winning:
            lines.append(f"💡 VCF冲四胜: {vcf_result[0].to_notation()}")

        my_patterns = engine.count_patterns(my_stone)
        opp_patterns = engine.count_patterns(opp_stone)

        if my_patterns[PatternType.OPEN_FOUR] > 0:
            lines.append(f"💪 己方活四/Open Four")
        if opp_patterns[PatternType.OPEN_FOUR] > 0:
            lines.append(f"⚠️ 对手活四/Opponent Open Four")

        if my_patterns[PatternType.RUSH_FOUR] > 0 and not my_winning:
            lines.append(f"⚔️ 己方冲四/Rush Four: {my_patterns[PatternType.RUSH_FOUR]}个")

        if my_patterns[PatternType.OPEN_THREE] > 0 and not my_winning:
            lines.append(f"进攻/Attack - 活三/Open Three: {my_patterns[PatternType.OPEN_THREE]}个")

        if opp_patterns[PatternType.OPEN_THREE] > 0 and not opp_winning:
            lines.append(
                f"防守/Defend - 对手活三/Opponent Open Three: {opp_patterns[PatternType.OPEN_THREE]}个"
            )

        if game.rule_mode.value == "renju" and my_stone == Stone.BLACK:
            fd = ForbiddenDetector(board)
            forbidden = fd.get_forbidden_positions()
            if forbidden:
                lines.append(
                    f"⛔ 禁手/Forbidden: {', '.join(p.to_notation() for p in forbidden[:5])}"
                )
    ...
    def get_move(self, game: Game) -> Optional[Position]:
        ...
        for attempt in range(self.max_retries):
            try:
                response = self.client.generate(prompt, max_tokens=50)
    ...
    def _build_prompt(self, game: Game) -> str:
        ...
        color_en = "BLACK (●)" if self.stone == Stone.BLACK else "WHITE (○)"

        threat_analysis = self._analyze_threats(game)
        candidates = self._get_candidates(game)
        candidates_info = (
            f"【推荐位置/Suggested moves】{', '.join(p.to_notation() for p in candidates)}"
            if candidates
            else ""
        )
